...
10. Run `pnpm caes-ai app list` and confirm `todo-app` is enabled with one active key ID. The command must not print an API key or hash.
...
The Playwright suite runs this live path with `pnpm test:e2e`; it requires the provider credential and can incur API usage. To run it against an already running Compose stack, use `CAES_AI_E2E_USE_EXISTING_STACK=true CAES_AI_E2E_BASE_URL=http://localhost:8080 pnpm test:e2e`.
